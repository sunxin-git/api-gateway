...
- **占位（提交前，与 task 落库同事务）**：
  ...
  WHERE (business_account_id, model) = ($1, $2) AND inflight < cap
  RETURNING inflight;
  ```
  `RETURNING` 拿到行 → 占位成功，继续落 task + 入队；无行 → 占不到 → **429**。用条件 `UPDATE ... RETURNING`（单条原子语句，PG 行锁串行化）而非「`SELECT COUNT FOR UPDATE` 再判」，规避幻读插入式 TOCTOU。
...
| `github.com/redis/go-redis/v9` | v9.20.0（Unit 1 落地） | BSD-2-Clause | Asynq 后端 | indirect（transitive） |

> **启动 ping fail-fast 实现说明（Unit 1）**：未单独引 go-redis 客户端做 ping，改用 asynq 自带的 `Server.Ping()`（同一 RedisConnOpt，等价探活），少一个直接依赖。故 go-redis 为 asynq 的**传递依赖（indirect）**，仍是 Redis 后端，license/合规不变。
| `github.com/volcengine/ve-tos-golang-sdk/v2` | ~v2.9.x | Apache-2.0 | TOS 结果转存 + 预签名 GET URL |
...
- ⚠️ 引入 3 个新依赖 + 运行期强依赖 Redis 可用性（启动 fail-fast，运维须保障 Redis）。
